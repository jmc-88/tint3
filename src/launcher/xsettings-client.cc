...
static int IgnoreErrors(Display* display, XErrorEvent* event) { return True; }
...
static XSettingsList* ParseSettings(unsigned char* data, size_t len) {
  ...
  if (buffer.byte_order != MSBFirst && buffer.byte_order != LSBFirst) {
    util::log::Error() << "Invalid byte order " << buffer.byte_order
...
static void ReadSettings(XSettingsClient* client) {
  ...
  int result =
      XGetWindowProperty(client->display, client->manager_window,
                         server.atoms_["_XSETTINGS_SETTINGS"], 0, LONG_MAX,
                         False, server.atoms_["_XSETTINGS_SETTINGS"], &type,
...
static void CheckManagerWindow(XSettingsClient* client) {
  ...
  client->manager_window =
      XGetSelectionOwner(server.dsp, server.atoms_["_XSETTINGS_SCREEN"]);
