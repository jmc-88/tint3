...
void remove_task (Task *tsk)
{
    ...
    GPtrArray* task_group = (GPtrArray*) g_hash_table_lookup(win_to_task_table, &win);
...
int get_title(Task *tsk)
{
    ...
    char* name = (char*) server_get_property(tsk->win, server.atom._NET_WM_VISIBLE_NAME, server.atom.UTF8_STRING, 0);
    if (!name || !strlen(name)) {
        name = (char*) server_get_property(tsk->win, server.atom._NET_WM_NAME, server.atom.UTF8_STRING, 0);
        if (!name || !strlen(name)) {
...
void get_icon (Task *tsk)
{
    ...
    data = (gulong*) server_get_property(tsk->win, server.atom._NET_WM_ICON, XA_CARDINAL, &i);
    if (data) {
        // get ARGB icon
        int w, h;
        gulong* tmp_data = get_best_icon(data, get_icon_count (data, i), i, &w, &h, panel->g_task.icon_size1);
#ifdef __x86_64__
