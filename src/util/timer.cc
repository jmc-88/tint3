...
void CreateMultiTimeout(Timeout* t1, Timeout* t2) {
  ...
  // set real_timeout->multi_timeout to something, such that we see in
  // add_timeout_intern that
  // it is already a multi_timeout (we never use it, except of checking for 0
  // ptr)
    ...
    if (t2->interval_msec > 0) {
      if (t->interval_msec % t2->interval_msec == 0 ||
        ...
        if (!t->multi_timeout && !t2->multi_timeout) {
          // both timeouts can be aligned, but there is no multi timeout for
          // them
          CreateMultiTimeout(t, t2);
        } else {
          // there is already a multi timeout, so we append the new timeout to
          // the multi timeout
...
void AddTimeoutInternal(int value_msec, int interval_msec,
  ...
  if (!can_align) {
    auto it = std::lower_bound(
        timeout_list.begin(), timeout_list.end(), t,
        [](Timeout* t1, Timeout* t2) { return CompareTimeouts(t1, t2); });
...
void RemoveFromMultiTimeout(Timeout* t) {
  ...
  mth->timeout_list.erase(
      std::remove(mth->timeout_list.begin(), mth->timeout_list.end(), t),
      mth->timeout_list.end());
  ...
  if (mth->timeout_list.size() == 1) {
    ...
    mth->timeout_list.erase(std::remove(mth->timeout_list.begin(),
                                        mth->timeout_list.end(), last_timeout),
                            mth->timeout_list.end());
...
void StopMultiTimeout(Timeout* t) {
  ...
  for (auto const& t1 : mth->timeout_list) {
    mth->timeout_list.erase(
        std::remove(mth->timeout_list.begin(), mth->timeout_list.end(), t1),
        mth->timeout_list.end());
...
*timeout in the list,
    * and callback_timeout_expired() only have to consider the timeouts as long
*as the expiration time
    * is in the past to the current time.
    * As time measurement we use clock_gettime(CLOCK_MONOTONIC) because this
*refers to a timer, which
    * reference point lies somewhere in the past and cannot be changed, but just
*queried.
    * If a single shot timer is installed it will be automatically deleted. I.e.
*the returned value
    * of add_timeout will not be valid anymore. You do not need to call
*stop_timeout for these timeouts,
...
void StopTimeout(Timeout* t) {
  bool has_timeout = (std::find(timeout_list.begin(), timeout_list.end(), t) !=
                      timeout_list.end());
  bool has_multi_timeout = (multi_timeouts.find(t) != multi_timeouts.end());
