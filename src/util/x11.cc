            ...
            for (int i = 0 ; i < nb_panel ; i++) {
                ...
                if (panel.is_hidden_) {
                    XCopyArea(server_->dsp,
                              ...
                              0, 0);
                    XSetWindowBackgroundPixmap(server_->dsp,
                                               panel.main_win_,
                                               panel.hidden_pixmap_);
                } else {
                    if (panel.temp_pmap) {
                    ...
                    panel.temp_pmap = XCreatePixmap(server_->dsp,
                                                    server_->root_win,
                                                    panel.width_,
                                                    panel.height_,
                                                    server_->depth);
                    panel.Render();
                    XCopyArea(server_->dsp, panel.temp_pmap, panel.main_win_,
                              server_->gc, 0, 0, panel.width_, panel.height_,
                              0, 0);
                }
            }
...
EventLoop& EventLoop::RegisterDefaultHandler(
    EventLoop::EventHandler handler) {
